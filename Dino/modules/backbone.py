...
from Dino.modules.model import _default_tfmer_cfg
from Dino.modules.resnet import resnet45, resnetfpn, resnetunet
from Dino.modules.resnetfpn import FPN50
from Dino.modules.transformer import (PositionalEncoding,
                                      TransformerEncoder,
...
class ResFPNTranformer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.resnet = resnetunet()
    ...
    def forward(self, images, mask_flag=False):
        feature, learned_mask = self.resnet(images, mask_flag)
        n, c, h, w = feature.shape
        feature = feature.view(n, c, -1).permute(2, 0, 1)
        feature = self.pos_encoder(feature)
        feature = self.transformer(feature)
        feature = feature.permute(1, 2, 0).view(n, c, h, w)
        return feature, learned_mask
